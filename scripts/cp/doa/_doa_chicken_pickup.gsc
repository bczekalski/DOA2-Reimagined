...
function private function_e4f21fa9()
{
	...
	if(roll < 2)
	{
		prize = level.doa.extra_life_model; //free man
	}
	else
	{
		if(roll < 6)
		{
			prize = level.doa.var_501f85b4; //nuke
		}
		else
		{
			if(roll < 16)
			{
				prize = level.doa.booster_model; //boost
			}
			else
			{
				if(roll <= 40)
				{
					prize = level.doa.var_8d63e734; //chicken
				}
				else
				{
					prize = "zombietron_diamond"; //gem
				}
			}
		}
	}
